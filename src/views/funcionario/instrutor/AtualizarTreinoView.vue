...
const buscarTreinoAtual = async () => {
  try {
    ...
    ativo.value = treino.ativo;
    exercicios.value = treino.exercicios || [];
  } catch (error) {
    console.error('Erro ao buscar treino:', error);
    alert('Treino não encontrado. Redirecionando...');
...
const validarEAtualizar = async () => {
  ...
  if (Object.keys(erros.value).length === 0) {
    try {
      ...
      router.push('/funcionario'); // Redireciona de volta
    } catch (error) {
      console.error('Erro ao atualizar treino:', error);
      alert('Ocorreu um erro ao atualizar o treino. Tente novamente.');
    }
  }
};

onMounted(() => {
  treinoId.value = route.params.id;
  if (!treinoId.value) {
    alert('ID do treino não fornecido na rota!');
    router.push('/funcionario');
    return;
  }
...
<template>
  <main>
    <div id="cabecalho">
      ...
      <p id="subtitulo">Edite os dados ou a ficha de exercícios do treino selecionado.</p>
    </div>
    ...
    <div v-else id="form">
      ...
      <div id="checkbox" class="campo">
        <label style="display: flex; align-items: center; gap: 8px; font-weight: normal; cursor: pointer;">
          <input type="checkbox" v-model="ativo" style="width: auto; margin: 0; cursor: pointer;" />
          Treino ativo (visível para o aluno)
        </label>
      </div>
      ...
      <!-- Botões de Ação -->
      <div id="botoes">
        <button type="submit" id="cadastrar" @click.prevent="validarEAtualizar">Salvar Alterações</button>
